...
CreateWindow( GtkApplication* app )
{
	...
	char*      title  = "Example GTK+ 3 Application";
	guint      width  = 480;
	guint      height = 360;
...
void
ActivateApplication( GtkApplication* app, gpointer user_data )
{
	GtkWidget* window = CreateWindow( app );
	GtkWidget* label  = gtk_label_new( NULL );
	char*      text   = "This window is created as application.\nApplication is destroyed, if the window is closed.";
...
main ( int argc, char** argv )
{
	...
	g_signal_connect( app, "activate", G_CALLBACK( ActivateApplication ), NULL);
