...
func New() (list []uint64) {
	return make([]uint64, 0, caparr.INITIAL_SIZE)
}

func Append(list []uint64, unsigned_integer uint64) (new_list []uint64) {
...
func AppendList(list1 []uint64, list2 []uint64) (new_list []uint64) {
...
func Insert(list []uint64, index uint64, value uint64) ([]uint64) {
	list_length := uint64(len(list))
	list_new_length := list_length+1
	list = caparr.Ensure(list, list_new_length)
...
func Remove(list []uint64, index uint64) ([]uint64) {
	list_length := uint64(len(list))
...
func LinearSearch(list []uint64, unsigned_integer uint64) (index uint64, match bool) {
